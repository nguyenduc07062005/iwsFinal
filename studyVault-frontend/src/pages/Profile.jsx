...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    const loadProfile = async () => {
      try {
        ...
        if (!active) return;
        setError(getApiErrorMessage(requestError, 'Could not load account information.'));
      } finally {
  ...
  const handleProfileSubmit = async (event) => {
    ...
    if (!name) {
      setProfileFeedback({ tone: 'error', message: 'Display name cannot be empty.' });
      return;
    }
    ...
    try {
      ...
      setProfileFeedback({
        tone: 'error',
        message: getApiErrorMessage(requestError, 'Could not update profile.'),
      });
    } finally {
  ...
  const handlePasswordSubmit = async (event) => {
    ...
    if (!passwordForm.currentPassword || !passwordForm.newPassword) {
      setPasswordFeedback({ tone: 'error', message: 'Please fill in all password fields.' });
      return;
    }

    if (passwordForm.newPassword.length < 6) {
      setPasswordFeedback({ tone: 'error', message: 'New password must be at least 6 characters.' });
      return;
    }
    ...
    try {
      ...
      setPasswordFeedback({
        tone: 'error',
        message: getApiErrorMessage(requestError, 'Could not update password.'),
      });
    } finally {
                ...
                <div className="flex min-w-0 items-center gap-4">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 flex min-w-0 items-center gap-2 truncate text-sm font-semibold text-slate-500">
                      <Mail size={15} />
                      <span className="truncate">{profile?.email || 'Loading email...'}</span>
                    </p>
                  </div>
                </div>
            ...
            {loading ? (
              ...
              <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_360px] 2xl:grid-cols-[minmax(0,1fr)_400px]">
                ...
                <aside className="space-y-4">
                  <InfoPill icon={UserRound} label="Role" value={getRoleLabel(profile?.role)} />
                  <InfoPill
                    icon={CheckCircle2}
                    label="Status"
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-sm font-semibold leading-6 text-slate-500">
                          Update your password when you need to improve account security.
                        </p>
                      </div>
                    </div>
                    <button
