...
const getStatusLabel = (isActive) => (isActive ? 'Hoạt động' : 'Đã khóa');
...
const Admin = () => {
  ...
  const loadAdminData = async () => {
    ...
    try {
      ...
      setError(getApiErrorMessage(requestError, 'Không tải được trang quản trị.'));
    } finally {
    ...
    return [
      {
        ...
        subLabel: `${formatNumber(usersStat.active)} hoạt động · ${formatNumber(usersStat.locked)} khóa`,
      },
      {
        icon: FileText,
        label: 'Tài liệu',
      ...
      },
      {
        ...
        subLabel: 'Tổng thư mục trong hệ thống',
      },
      {
        icon: HardDrive,
        label: 'Dung lượng',
        value: formatBytes(storageStat.totalBytes),
        subLabel: 'Tổng dung lượng đã lưu',
      },
    ];
  }, [stats]);
  ...
  const handleToggleStatus = async (user) => {
    try {
      ...
      setError(getApiErrorMessage(requestError, 'Không cập nhật được trạng thái tài khoản.'));
    } finally {
      setUpdatingUserId('');
    }
  };
          ...
          <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div className="min-w-0">
              ...
              <p className="mt-2 max-w-2xl text-sm font-semibold leading-6 text-slate-500">
                Theo dõi tài khoản, trạng thái người dùng và quy mô dữ liệu của StudyVault.
              </p>
            </div>
                      ...
                      <tr key={user.id} className="transition-colors hover:bg-white/45">
                        ...
                        <td className="px-5 py-4 text-right">
                          <button
                            ...
                            title={
                              user.role === 'admin'
                                ? 'Không khóa tài khoản admin từ giao diện này'
                                : user.isActive
                                  ? 'Khóa tài khoản'
                                  : 'Mở tài khoản'
                            }
